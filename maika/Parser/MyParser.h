...
#line 149 "Parser/MyParser.h" // lalr1.cc:392
  ...
  public:
    ...
    union union_type
    {
      ...
      // "double_literal"
      char dummy8[sizeof(std::shared_ptr<DoubleLiteral>)];

      // literal
      // primary_expression
      // expression
      char dummy9[sizeof(std::shared_ptr<Expr>)];

      // for_statement
      char dummy10[sizeof(std::shared_ptr<ForStmt>)];

      // function_definition
      char dummy11[sizeof(std::shared_ptr<FunctionDecl>)];

      // function_expression
      char dummy12[sizeof(std::shared_ptr<FunctionExpr>)];

      // if_statement
      char dummy13[sizeof(std::shared_ptr<IfStmt>)];

      // "integer_literal"
      char dummy14[sizeof(std::shared_ptr<IntegerLiteral>)];

      // map_entry
      char dummy15[sizeof(std::shared_ptr<MapEntry>)];

      // map_literal
      char dummy16[sizeof(std::shared_ptr<MapLiteral>)];

      // member_expression
      char dummy17[sizeof(std::shared_ptr<MemberExpr>)];

      // "identifier"
      // binding_identifier
      // type_specifier
      char dummy18[sizeof(std::shared_ptr<NamedDecl>)];

      // null_literal
      char dummy19[sizeof(std::shared_ptr<NullLiteral>)];

      // parameter_variable
      char dummy20[sizeof(std::shared_ptr<ParmVarDecl>)];

      // return_statement
      char dummy21[sizeof(std::shared_ptr<ReturnStmt>)];

      // statement
      // for_init_statement
      char dummy22[sizeof(std::shared_ptr<Stmt>)];

      // "string_literal"
      char dummy23[sizeof(std::shared_ptr<StringLiteral>)];

      // subscript_expression
      char dummy24[sizeof(std::shared_ptr<SubscriptExpr>)];

      // translation_unit
      char dummy25[sizeof(std::shared_ptr<TranslationUnitDecl>)];

      // unary_expression
      char dummy26[sizeof(std::shared_ptr<UnaryOperator>)];

      // variable_definition
      char dummy27[sizeof(std::shared_ptr<VariableDecl>)];

      // while_statement
      char dummy28[sizeof(std::shared_ptr<WhileStmt>)];

      // expression_list
      char dummy29[sizeof(std::vector<std::shared_ptr<Expr>>)];

      // function_definitions
      char dummy30[sizeof(std::vector<std::shared_ptr<FunctionDecl>>)];

      // map_entry_list
      char dummy31[sizeof(std::vector<std::shared_ptr<MapEntry>>)];

      // parameter_variables
      char dummy32[sizeof(std::vector<std::shared_ptr<ParmVarDecl>>)];

      // statement_list
      char dummy33[sizeof(std::vector<std::shared_ptr<Stmt>>)];
};
  ...
  private:
    ...
    {
      yyeof_ = 0,
      yylast_ = 796,     ///< Last index in yytable_.
...
#line 2364 "Parser/MyParser.h" // lalr1.cc:392
