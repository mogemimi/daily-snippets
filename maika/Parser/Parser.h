...
#line 149 "Parser/Parser.h" // lalr1.cc:392
  ...
  public:
    ...
    {
      // binary_op
      char dummy1[sizeof(BinaryOperatorKind)];

      // call_signature
      char dummy2[sizeof(CallSignature)];

      // variable_decl_specifier
      char dummy3[sizeof(VariableDeclSpecifier)];

      // array_literal
      char dummy4[sizeof(std::shared_ptr<ArrayLiteral>)];

      // binary_expression
      // assignment_expression
      char dummy5[sizeof(std::shared_ptr<BinaryOperator>)];

      // binding_declaration
      char dummy6[sizeof(std::shared_ptr<BindingDecl>)];

      // "bool_literal"
      char dummy7[sizeof(std::shared_ptr<BoolLiteral>)];

      // call_expression
      char dummy8[sizeof(std::shared_ptr<CallExpr>)];

      // class_declaration
      char dummy9[sizeof(std::shared_ptr<ClassDecl>)];

      // compound_statement
      char dummy10[sizeof(std::shared_ptr<CompoundStmt>)];

      // conditional_expression
      char dummy11[sizeof(std::shared_ptr<ConditionalOperator>)];

      // translation_unit_declaration
      // member_declaration
      // for_range_init
      char dummy12[sizeof(std::shared_ptr<Decl>)];

      // decomposition_definition
      char dummy13[sizeof(std::shared_ptr<DecompositionDecl>)];

      // defer_statement
      char dummy14[sizeof(std::shared_ptr<DeferStmt>)];

      // "double_literal"
      char dummy15[sizeof(std::shared_ptr<DoubleLiteral>)];
      ...
      // for_range_statement
      char dummy17[sizeof(std::shared_ptr<ForRangeStmt>)];

      // for_statement
      char dummy18[sizeof(std::shared_ptr<ForStmt>)];

      // function_definition
      char dummy19[sizeof(std::shared_ptr<FunctionDecl>)];

      // function_expression
      char dummy20[sizeof(std::shared_ptr<FunctionExpr>)];

      // if_statement
      char dummy21[sizeof(std::shared_ptr<IfStmt>)];

      // "integer_literal"
      char dummy22[sizeof(std::shared_ptr<IntegerLiteral>)];

      // map_entry
      char dummy23[sizeof(std::shared_ptr<MapEntry>)];

      // map_literal
      char dummy24[sizeof(std::shared_ptr<MapLiteral>)];

      // member_expression
      char dummy25[sizeof(std::shared_ptr<MemberExpr>)];

      // "identifier"
      // binding_identifier
      // type_specifier
      char dummy26[sizeof(std::shared_ptr<NamedDecl>)];

      // null_conditional_expression
      char dummy27[sizeof(std::shared_ptr<NullConditionalOperator>)];

      // null_literal
      char dummy28[sizeof(std::shared_ptr<NullLiteral>)];

      // parenthesized_expression
      char dummy29[sizeof(std::shared_ptr<ParenExpr>)];

      // parameter_variable
      char dummy30[sizeof(std::shared_ptr<ParmVarDecl>)];

      // return_statement
      char dummy31[sizeof(std::shared_ptr<ReturnStmt>)];

      // statement
      // for_init_statement
      char dummy32[sizeof(std::shared_ptr<Stmt>)];

      // "string_literal"
      char dummy33[sizeof(std::shared_ptr<StringLiteral>)];

      // subscript_expression
      char dummy34[sizeof(std::shared_ptr<SubscriptExpr>)];

      // translation_unit
      char dummy35[sizeof(std::shared_ptr<TranslationUnitDecl>)];

      // unary_expression
      char dummy36[sizeof(std::shared_ptr<UnaryOperator>)];

      // variable_definition
      char dummy37[sizeof(std::shared_ptr<VariableDecl>)];

      // while_statement
      char dummy38[sizeof(std::shared_ptr<WhileStmt>)];

      // binding_declarations
      char dummy39[sizeof(std::vector<std::shared_ptr<BindingDecl>>)];

      // translation_unit_declarations
      // member_declaration_list
      char dummy40[sizeof(std::vector<std::shared_ptr<Decl>>)];

      // expression_list
      char dummy41[sizeof(std::vector<std::shared_ptr<Expr>>)];

      // map_entry_list
      char dummy42[sizeof(std::vector<std::shared_ptr<MapEntry>>)];

      // parameter_variables
      char dummy43[sizeof(std::vector<std::shared_ptr<ParmVarDecl>>)];

      // statement_list
      char dummy44[sizeof(std::vector<std::shared_ptr<Stmt>>)];
};
  ...
  basic_symbol (typename Base::kind_type t, const BinaryOperatorKind v, const location_type& l);

  basic_symbol (typename Base::kind_type t, const CallSignature v, const location_type& l);

  basic_symbol (typename Base::kind_type t, const VariableDeclSpecifier v, const location_type& l);
    ...
    , location (other.location)
  {
    ...
    {
      case 107: // binary_op
        value.copy< BinaryOperatorKind > (other.value);
        break;
      ...
      case 89: // variable_decl_specifier
        value.copy< VariableDeclSpecifier > (other.value);
        break;
    ...
    , location (l)
  {
    ...
    {
      case 107: // binary_op
        value.copy< BinaryOperatorKind > (v);
        break;
      ...
      case 89: // variable_decl_specifier
        value.copy< VariableDeclSpecifier > (v);
        break;
  ...
  template <typename Base>
  Parser::basic_symbol<Base>::basic_symbol (typename Base::kind_type t, const BinaryOperatorKind v, const location_type& l)
    : Base (t)
    , value (v)
    , location (l)
  {}
  ...
  template <typename Base>
  Parser::basic_symbol<Base>::basic_symbol (typename Base::kind_type t, const VariableDeclSpecifier v, const location_type& l)
    : Base (t)
    , value (v)
    , location (l)
  {}
  ...
  Parser::basic_symbol<Base>::clear ()
  {
   ...
   default:
    ...
    {
      case 107: // binary_op
        value.template destroy< BinaryOperatorKind > ();
        break;
      ...
      case 89: // variable_decl_specifier
        value.template destroy< VariableDeclSpecifier > ();
        break;
  ...
  Parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    {
      case 107: // binary_op
        value.move< BinaryOperatorKind > (s.value);
        break;
      ...
      case 89: // variable_decl_specifier
        value.move< VariableDeclSpecifier > (s.value);
        break;
...
#line 2755 "Parser/Parser.h" // lalr1.cc:392
